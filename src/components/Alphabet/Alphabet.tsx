...
const KeyboardLetter = ({ index, isSelected, onSelect }: KeyboardLetterProps) => {
    ...
    return (
        <div className={className} key={`l${index}`} onClick={() => handleClick(index)}>
            {LETTERS[index].letter}
        </div>
    )
}
    ...
    return (
        <div className={styles.wrapper}>
            ...
            <div className={styles.buttons}>
                {LETTERS.map((l, i) => (
                    <KeyboardLetter index={i} isSelected={i === index} onSelect={i => setIndex(i)} />
                ))}
            </div>
        </div>
    );
